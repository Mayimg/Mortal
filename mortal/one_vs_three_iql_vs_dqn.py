...
def main():
    ...
    # Champion: model trained by mortal/train.py (Brain + DQN)
    if use_akochan:
        ...
        mortal_cham = Brain(version=version, conv_channels=conv_channels, num_blocks=num_blocks).eval()
        dqn_cham = DQN(version=version).eval()
        mortal_cham.load_state_dict(state['mortal'])
    ...
    # Challenger: model trained by mortal/train_iql_offline.py (Brain + PolicyNet w/ IQL)
    state = torch.load(
        cfg['challenger']['state_file'],
        weights_only=True,
        map_location=torch.device('cpu'),
    )
    ...
    mortal_chal = Brain(version=version, conv_channels=conv_channels, num_blocks=num_blocks).eval()
    policy_chal = PolicyNet().eval()
    mortal_chal.load_state_dict(state['mortal'])
    if 'policy' in state:
        policy_chal.load_state_dict(state['policy'])
    # Ensure engine recognizes IQL policy
    policy_chal.policy_mode = 'iql'
    ...
    for i, seed in enumerate(range(seed_start, seed_start + seeds_per_iter * iters, seeds_per_iter)):
        ...
        print(f'challenger (IQL) rankings: {rankings} ({avg_rank}, {avg_pt}pt)')
